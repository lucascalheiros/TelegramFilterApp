...
class FilterMessagesReducer @Inject constructor(): Reducer<FilterMessagesUiState, FilterMessagesAction> {
    override fun reduce(
        state: FilterMessagesUiState,
        action: FilterMessagesAction
    ): FilterMessagesUiState {
        return when (action) {
            is FilterMessagesAction.SetMessages -> state.copy(messages = action.messages)
        }
    }
}